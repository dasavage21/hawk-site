// This module provides the self-contained chat widget embed script.
// It's served as a static JS file at /api/embed/chat-embed.js

/** Generate the embed script as a string for serving */
function makeEmbedScript(): string {
...
var sid='lac_'+crypto.randomUUID(),open=false,_init=false,msgs=[];
...
function am(r,t){msgs.push({role:r,content:t});var d=document.createElement('div');d.className='lac-msg '+(r==='user'?'u':'b');d.textContent=t;mc.appendChild(d);mc.scrollTop=mc.scrollHeight}
...
b.addEventListener("click",function(){b.style.display="none";w.classList.add("o");_init()});
xc.addEventListener("click",function(){w.classList.remove("o");b.style.display="flex"});
