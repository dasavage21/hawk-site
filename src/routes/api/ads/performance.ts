  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (conditions.length > 0) {
          // We need to filter variationsQuery as well, but some conditions (like date) might be different
          // Actually, we want performance of variations WITHIN that date range
